...
# Package meta-data.
NAME = "SIRT6"
DESCRIPTION = "Lab Final Project"

# These can be set to None if you want to omit them
URL = "None"
AUTHOR = "Sirtuin 6 - Final Project"
AUTHOR_EMAIL = "rshtirme@ramapo.edu"
LICENSE = "MIT license"
...
if not VERSION:
    with open(
        os.path.join(here, "src", "sirt6", "__version__.py")
    ) as f:
        exec(f.read(), about)
else:
...
setup(
    ...
    scripts=["bin/run-sirt6-prod"],
    entry_points={
        "console_scripts": [
            "run-sirt6-dev=sirt6.dev_cli:main"
        ]
    },
)
